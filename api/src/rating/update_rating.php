...
if(
     ...
     !empty($data->user_id) &&
     !empty($data->user_rating) &&
     !empty($data->business_owner) &&
     !empty($data->user_review) 
)

{
    $insert_rating->user_name = $data->user_name;
    $insert_rating->user_id = $data->user_id;
    $insert_rating->business_owner = $data->business_owner;
    $insert_rating->user_rating = $data->user_rating;
    $insert_rating->user_review = $data->user_review;
    $insert_rating->created_on = $data->created_on;
    
    //var_dump($insert_rating);
    // create the insert_rating
    if($insert_rating->update_rating()){

        http_response_code(201);
        echo json_encode(array("message" => "successfully_updated"));
    }
    else{
